...
def save_model(epoch, model_state_params, optimizer_state_param, scheduler_state_param, loss):
    ...
    path_remote = '/data/leuven/329/vsc32927/Thesis_varnet/redo/marginpt2/' + dt_string + '_base.ckpt'
    torch.save({
        'epoch': epoch,
...
def set_variance(embeds, outfit_sizes):
    ...
    for i in outfit_splits:
        set_rep.append(torch.mean(torch.var(i, dim=0)))
...
def varnet(model, dataloader, valid_loader, optimizer, scheduler, epochs, transforms, device, validating):
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch, (outfit_ids, len_outfits, outfit_labels) in enumerate(dataloader):
            ...
            if (batch+1)%20 ==0:
                print('Epoch {}, Step {}/{}, Pairs: {}/{}, Pos_Pair_Loss: {:.3f}, Overall_Loss: {:.3f}, Consis_var: {:.3f}, Inconsis_var: {:.3f}, Embeds Norm: {:.1f}'.format(epoch, (batch + 1), len(dataloader), pos_pairs, valid_pairs, loss, overall_loss, consis_var, inconsis_var, mean_embeds_norm))
            ...
            if ((batch + 1) % 100) == 0:
                now = datetime.now()
                dt_string = now.strftime("%d-%m-%Y_%H:%M:%S")
                bpath_remote = '/data/leuven/329/vsc32927/Thesis_varnet/redo/marginpt2/ckpt/' + dt_string + '_epoch' + str(epoch) + '_batch' + str(batch) + '.ckpt'
