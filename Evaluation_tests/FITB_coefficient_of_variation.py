...
PATH='/home/preyas/Desktop/Thesis/Thesis_models/Thesis_varcoeff_redo/margin3/21-02-2020_18:47:38_base.ckpt'
...
def fitb_accuracy_varcoeff(fitb_pairs):
    ...
    for q_index, (questions_list, answers_list, correct_idx) in enumerate(fitb_pairs):
        ...
        for a_index, answer in enumerate(answers_list):
            ...
            set_std = torch.std(set, dim=0)
            set_abs_mean = torch.mean(torch.abs(set), dim=0)
            set_varcoeff = torch.mean(torch.div(set_std, set_abs_mean))
            answer_score[a_index] = set_varcoeff.item()
...
acc_varcoeff=fitb_accuracy_varcoeff(fitb_pairs=qa_pairs)
print('Varcoeff Accuracy is {:.4f}'.format(acc_varcoeff))
